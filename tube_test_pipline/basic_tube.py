...
def get_bottom_endpoint(mask: np.ndarray, z_min: int, z_max: int, height_fraction: float, block_size: int, placement: str):
...
def trace_path_through_edt(edt_map: np.ndarray, top_point: Tuple[int, int, int], bottom_point: Tuple[int, int, int], search_radius: int) -> Tuple[List[float], List[float], List[int]]:
...
def smooth_path(path_x: List[float], path_y: List[float], path_z: List[int], sigma: float) -> Tuple[np.ndarray, np.ndarray, List[int]]:
...
def extract_centerline(mask: np.ndarray, top_point: Tuple[int, int, int], bottom_point: Tuple[int, int, int], search_radius: int, smoothing_sigma: float):
    """
...
def apply_separable_conv3d(volume_tensor: torch.Tensor, gaussian_weights: torch.Tensor, half_kernel: int) -> torch.Tensor:
    ...
    v = F.conv3d(F.pad(v, (0, 0, 0, 0, half_kernel, half_kernel), mode='replicate'), gaussian_weights.view(1, 1, -1, 1, 1))
    v = F.conv3d(F.pad(v, (0, 0, half_kernel, half_kernel, 0, 0), mode='replicate'), gaussian_weights.view(1, 1, 1, -1, 1))
    v = F.conv3d(F.pad(v, (half_kernel, half_kernel, 0, 0, 0, 0), mode='replicate'), gaussian_weights.view(1, 1, 1, 1, -1))
...
def create_solid_path(shape: Tuple[int, int, int], path_x: List[float], path_y: List[float], path_z: List[int], diameter: float, device: torch.device) -> torch.Tensor:
...
def hollow_out_solid(solid_volume: torch.Tensor, thickness: float, tube_intensity: float, device: torch.device) -> torch.Tensor:
...
def build_tube_gpu(shape: Tuple[int, int, int], path_x: List[float], path_y: List[float], path_z: List[int], 
                   diameter: float, thickness: float, shave_sigma: float, tube_intensity: float, device: torch.device) -> Tuple[np.ndarray, np.ndarray]:
