...
# ================== CONFIGURATION ==================
TUBE_WEIGHTS_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "models", "tube_weights.pth")
...
def process_patient(patient_id, model, rcg_cmap, model_type):
    ...
    for i in range(num_transitions):
        ...
        for trim, frac in cfg.TUBE_CROP_SETTINGS:
        ...
        base_trim, base_frac = cfg.TUBE_BASE_CROP
...
def run_batch_inference():
    ...
    # 1. Load Tube Model
    tube_model = VGGDiffNet()
    tube_model.load_state_dict(torch.load(TUBE_WEIGHTS_PATH, map_location=DEVICE, weights_only=True))
    tube_model.to(DEVICE).eval()
    ...
    for patient_id in patient_dirs:
        # Run Tube Inference
        process_patient(patient_id, tube_model, rcg_cmap, model_type="tube")

    print(f"\n{'=' * 60}")
    print(f"BATCH PROCESSING COMPLETE. Outputs saved in '{OUTPUT_DIR}/tube'.")
    print(f"{'=' * 60}")
