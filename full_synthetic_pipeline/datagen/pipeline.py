...
from datagen.deformation import get_deformed_tube_fast
...
def correct_mask_by_seg(mask, seg):
    """Intersects the deformed tube mask with the airway+lung segmentation."""
...
def add_tube(data, seg, pos, tube_length, outer_radius, inner_radius, margin):
    """
    Generates a deformed ET tube, smooths its boundary via pooling,
    and applies it to the volume.

    This replaces the old add_mass() which generated deformed spheres.
    The function signature changed to accept tube geometry parameters
    instead of a single sphere radius.
    """
    ...
    print("  Running get_deformed_tube_fast...")

    deformed_tube, mask = get_deformed_tube_fast(
        working_data,
        cfg.TUBE_WALL_INTENSITY,
        cfg.TUBE_LUMEN_INTENSITY,
        pos, tube_length, outer_radius, inner_radius, margin,
    )

    mask = correct_mask_by_seg(mask, seg)
    apply_mask(working_data, deformed_tube, mask)
    print("  Finished deformed tube.")

    # boundary smoothing via pooling (uses smaller kernel for thin tube walls)
    print("  Start pooling...")
    pooled_data, mask = apply_pooling(working_data, mask, cfg.TUBE_POOLING_KERNEL_SIZE)
    mask = correct_mask_by_seg(mask, seg)
...
def create_prior_ct(prior, seg, prior_pos, tube_length, outer_radius, inner_radius, margin):
    """Adds a deformed ET tube to the prior CT volume."""
    working_data, mask = add_tube(prior, seg, prior_pos,
                                  tube_length, outer_radius, inner_radius, margin)
    apply_mask(prior, working_data, mask)
    return prior


def create_current_ct(current, seg, current_pos, tube_length, outer_radius, inner_radius, margin):
    """Adds a deformed ET tube to the current CT volume."""
    working_data, mask = add_tube(current, seg, current_pos,
                                  tube_length, outer_radius, inner_radius, margin)
...
def pipeline(pair_dir, ct_data, lungs_mask, tube_length, outer_radius, inner_radius,
    ...
    The `radius` parameter from the old sphere pipeline is replaced with
    tube_length, outer_radius, inner_radius to describe ET-tube geometry.
    """
    ...
    margin = max(outer_radius, 10)  # deformation headroom around the tube
    ...
    if has_current_mass:
        current_data = create_current_ct(current_data, seg, current_pos,
                                         tube_length, outer_radius, inner_radius, margin)
    ...
    if has_prior_mass:
        prior_data = create_prior_ct(prior_data, seg, prior_pos,
                                     tube_length, outer_radius, inner_radius, margin)
