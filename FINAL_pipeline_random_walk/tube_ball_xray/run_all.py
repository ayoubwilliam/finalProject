...
def run_with_retry(func, *args, **kwargs):
    """
    Runs a function in an isolated process to survive hard C-level Segmentation Faults.
    If the process crashes (exit code != 0), it clears the GPU memory and retries.
    """
    max_retries = 3
    for attempt in range(1, max_retries + 1):
        ...
        print(f"\n  [WARNING] Crash detected (Exit code {p.exitcode}) on attempt {attempt}/{max_retries}!")
        
        # Force flush the GPU memory before retrying
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
            
        if attempt == max_retries:
            print("  -> Giving up on this scan after maximum retries.")
            return False
...
def run_segmentation(scan_files):
    ...
    for filepath in scan_files:
        ...
        if os.path.exists(seg_path):
            ...
            with tempfile.TemporaryDirectory(prefix=f"totseg_{filename}_") as tmp_dir:
                ...
                if not success:
                    print(f"  Skipping {input_path} due to repeated crashes or permissions.")
                    continue
    ...
    if cfg.ADD_TUBE:
        ...
        for filepath in scan_files:
            ...
            if os.path.exists(trachea_seg_path):
                ...
                success = run_with_retry(generate_full_airway_mask, input_path, cfg.SEGMENTATION_DIR)
                if not success:
                    print(f"  -> Error generating trachea seg for {filename}")
...
if __name__ == "__main__":
    # Use 'spawn' so CUDA initializes freshly inside the isolated process,
    # preventing context-sharing SegFaults on Linux.
