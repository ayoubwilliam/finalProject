...
def train_one_batch(model, prior, current, target_angle, optimizer, criterion, scaler):
    """Executes a single forward/backward pass with AMP."""
    prior, current, target_angle = prior.to(cfg.DEVICE), current.to(cfg.DEVICE), target_angle.to(cfg.DEVICE)
    optimizer.zero_grad()
    with autocast():
        loss = criterion(model(prior, current), target_angle)
...
def train():
    ...
    criterion, scaler = nn.L1Loss(), GradScaler()
    ...
    for epoch in range(cfg.EPOCHS):
        model.train()
        running_loss = 0.0
        for prior, current, target_angle, filename, _ in train_loader:
            loss = train_one_batch(model, prior, current, target_angle, optimizer, criterion, scaler)
            running_loss += loss
            pbar.update(1)
