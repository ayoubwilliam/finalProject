# 📂 Codebase Structure Guide

This guide provides an exhaustive, one-to-one mapping of every file and directory present in the main GitHub repository. The project is modularized into four primary domains: the core twin-encoder model (main_model), a standard classification comparator (aseline_model), clinical demonstration orchestrators (demos), and dataset acquisition tools (data/real_xray).

---

## 1. 🧠 Main Model (main_model/)
This is the core directory of the project, containing the full End-to-End procedural data generation, PyTorch training, and Connected Components evaluation pipeline for the **VGGDiffNet** twin-encoder architecture.

### Root Level
- **config.py**: The universal configuration file defining absolute paths, synthesis hyperparameters, model learning rates, and tunable constants.
- **ile_handler.py**: Utility library for safely parsing, validating, loading, and saving multi-dimensional medical imaging files.
- **project_paths.py**: A helper that standardizes directory navigation and path resolution relative to the repository root.
- **
un_all.py**: The master orchestrator script that chains Data Generation, Training, and Evaluation sequentially.
- **
un_datagen.py**: Standalone executor to trigger only the 3D-to-2D data synthesis pipeline.
- **
un_training.py**: Standalone executor to trigger only the PyTorch neural network optimization loop.
- **
un_evaluation.py**: Standalone executor to evaluate the trained model on unseen testing data.
- **seg_generator.py**: Invokes the TotalSegmentator library to automatically generate essential organ bounding segmentations from raw 3D CT scans.

### datagen/ (Procedural Synthesis)
- **__init__.py**: Python package initializer.
- **generator.py**: Iterates over the raw dataset, manages patient files, and delegates synthesis tasks.
- **pipeline.py**: The primary data generation logic that sequentially processes a single 3D CT pair and manages memory.
- **crop.py**: Optimizes synthesis by applying tight bounding-box crops to isolate the lungs in 3D.
- **drr.py**: Applies Digitally Reconstructed Radiography, collapsing 3D CT volumes into 2D synthetic X-rays.
- **deformation.py**: Generates B-spline transformations to warp baseline spheres into organic, irregular shapes.
- **pooling.py**: Applies 3D volume pooling to smooth and blend the generated 3D anomalies.
- **shapes.py**: Core mathematical functions for generating base 3D spherical consolidations.
- **
otation.py**: Applies 3D volume rotations independently to prior and current scans to simulate patient posture shifts.
- **	ube.py**: Core logic for generating synthetic Endotracheal (ET) tubes in 3D space.
- **	ube_randomization.py**: Positions and curves the ET tubes using Euclidean Distance Transforms (EDT) and random walks within the trachea.
- **
oise.py**: Injects statistical artifacts and Gaussian noise to simulate real-world X-ray sensor degradation.

### 	raining/ (Deep Learning)
- **__init__.py**: Python package initializer.
- **models.py**: Defines the VGGDiffNet architecture, the weight-sharing twin-encoders, and the feature-subtraction bottleneck.
- **	rainer.py**: The core PyTorch training loop, including loss calculations, backpropagation, and mixed-precision scaling.
- **data_loader.py**: Custom PyTorch Dataset classes for batched fetching of generated 2D X-ray image pairs.
- **model_io.py**: Safely manages saving, loading, and checkpointing neural network .pth weight files.

### evaluation/ (Testing & Metrics)
- **__init__.py**: Python package initializer.
- **evaluate.py**: The top-level orchestrator that evaluates the network on the test dataset.
- **cc_lib.py**: Implements the core mathematical algorithms for Object-Level Connected Components (CC) analysis.
- **cc_metrics.py**: Calculates spatial Intersection-over-Union (IoU) and bounding box overlaps for accurate True Positive/False Negative spatial scoring.
- **cc_dice.py**: Calculates standard object-level Dice coefficients to measure spatial prediction overlap.
- **cc_visuals.py & isuals.py**: Generates intuitive PNG comparison grids showing the Prior X-ray, Current X-ray, Ground Truth, and the AI's Predicted Heatmap side-by-side.

### lib/ (Helpers)
- **__init__.py**: Python package initializer.
- **
ifti_io.py**: Highly optimized helper functions for reading and writing .nii.gz arrays to disk.
- **overlay_heatmap.py**: Applies colormaps (e.g., JET) to grayscale model outputs to create visual anomaly overlays.

---

## 2. ⚖️ Baseline Model (aseline_model/)
This directory contains a standard classifier architecture used as a baseline comparator to prove the efficacy of the twin-encoder VGGDiffNet. It has a parallel but simplified structure.

### Root Level
- **config.py**: Configurations specifically tailored for the baseline model's training and data sizes.
- **
un_all.py**: Orchestrates data generation and training for the baseline classifier.
- **
un_pipeline_on_single_test.py**: Utility to test a single image through the trained baseline model.

### datagen/ (Baseline Synthesis)
- **__init__.py, drr.py, generator.py, pipeline.py, 
otation.py, 	ube.py**: Mirrored scripts from the main model, tailored to generate standard non-paired classification datasets instead of temporal pairs.

### 	raining/ (Baseline Deep Learning)
- **__init__.py, data_loader.py, model_io.py, models.py, 	rainer.py**: Contains the standard ResNet/VGG baseline architectures and binary-cross-entropy (BCE) training loops designed for standard image classification.

### evaluation/
- **__init__.py, evaluate.py, isuals.py**: Calculates baseline classification metrics (Accuracy, F1, ROC-AUC) without spatial CC analysis.

### segmentation/ & lib/
- **seg_generator.py & 
ifti_io.py**: Mirrored helpers tailored for the baseline's distinct directory paths.

---

## 3. 🎮 Demos (demos/)
This directory contains the code to execute clinical inference using pre-trained weights on both pre-generated synthetic datasets and real-world clinical datasets.

### synthetic_data_demo/
Evaluates the model on synthetic 2D DRR X-ray pairs (Demo 2).
- **config.py**: Defines input/output paths for the synthetic test set.
- **pull_data.py**: A helper to retrieve remote synthetic data if not stored locally.
- **
un_all.py**: Orchestrator that triggers both Tube and Consolidation evaluation sub-packages.
- **consolidation_pkg/**: Contains preprocessing.py (formats the inputs) and 
un_consolidation.py (runs inference specifically for lung consolidations).
- **	ube_pkg/**: Contains preprocessing.py (formats the inputs) and 
un_tube.py (runs inference specifically for endotracheal tubes).
- **lib/**: Contains overlay_heatmap.py for grid visualization.

### 
eal_data_demo/
Evaluates the model on real-world longitudinal patient visits from the ICU (Demo 3).
- **
un_all.py**: Orchestrator that triggers inference on the chronologically ordered real-world COVID-19 dataset.
- **consolidation_pkg/ & 	ube_pkg/**: Parallel to the synthetic packages, but preprocessing.py is specialized to handle and crop real clinical DICOM/JPEG X-rays rather than synthetic DRRs.
- **lib/**: Contains overlay_heatmap.py for grid visualization.

---

## 4. 📂 Real Data Acquisition (data/real_xray/)
Tools for fetching and filtering external clinical datasets.

- **pull_data.py**: A standalone dataset parser. It connects to the public IEEE8023 COVID-19 Chest X-ray repository, downloads the clinical metadata.csv, and programmatically filters patients to find longitudinal sequences exhibiting dynamic intubations or consolidations. It then automatically downloads the corresponding raw X-ray images and clinical notes into structured directories for testing in Demo 3.
